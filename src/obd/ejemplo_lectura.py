...
if len(sys.argv) > 1:
    ...
    respuesta = input("¿Deseas usar el emulador OBD-II? (s/n): ")
    ...
    try:
        if USE_EMULADOR_IPC:
            ...
            shared_dict = manager.dict(
                {"modo": "ralenti", "falla": None, "rpm": 800, "vel": 0}
            )
        ...
        while True:
            ...
            if resp_rpm and "410C" in resp_rpm:
                ...
                if len(hexdata) == 4 and all(
                    c in "0123456789ABCDEFabcdef" for c in hexdata
                ):
                    try:
                        A = int(hexdata[:2], 16)
                        B = int(hexdata[2:], 16)
                        ...
                        error_log.append(
                            f"Error decodificando RPM: {e} | hexdata={hexdata}"
                        )
                        rpm_val = None
                else:
            ...
            if rpm_val is not None and 400 <= rpm_val <= 7000:
                if last_rpm is not None:
                    if abs(rpm_val - last_rpm) > 2000:
                        error_log.append(
                            f"Salto abrupto de RPM: {last_rpm} -> {rpm_val}"
                        )
                        rpm_val = last_rpm
                last_rpm = rpm_val
                rpm = rpm_val
            ...
            if resp_speed and "410D" in resp_speed:
                ...
                if len(hexdata) == 2 and all(
                    ...
                    try:
                        ...
                        error_log.append(
                            f"Error decodificando velocidad: {e} | hexdata={hexdata}"
                        )
                        speed_val = None
                else:
                ...
                error_log.append(
                    f"Respuesta velocidad incompleta/corrupta: {resp_speed}"
                )
                speed_val = None
